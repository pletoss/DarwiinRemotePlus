...
- (id) init
{
	...
	if (self != nil) {
		...
		// create a thread for processing the incoming IRData
        
        [NSThread detachNewThreadSelector:@selector(run) toTarget:[WiiMoteIRMotionDetector sharedInstance] withObject:nil];
        
        [[WiiMoteIRMotionDetector sharedInstance] setWiimote:self];
...
//		  irData[3].x, irData[3].y, irData[3].s);
    ...
    [[WiiMoteIRMotionDetector sharedInstance] addIRData:irData];
    
    //[self updateTrackedPosition];
    
    IRData lastPos = [[WiiMoteIRMotionDetector sharedInstance] getLastKnownPoint];

    NSLog(@"lastPos=(%d,%d)", lastPos.x, lastPos.y);
    
    float px = 1 - lastPos.x / 1024.0, py = 1 - lastPos.y / 768.0;
    
	if ([_delegate respondsToSelector:@selector (irPointMovedX:Y:)])
		[_delegate irPointMovedX:px Y:py];
